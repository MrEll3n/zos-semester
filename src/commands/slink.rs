...
pub fn handle_argv(argv: &[&str], context: &mut Context) {
    ...
    if let Err(e) = fs.dir_add_entry(&mut parent_inode, &name, inode_id) {
        ...
        let _ = fs.free_inode(inode_id);
        match e.kind() {
            ErrorKind::AlreadyExists => println!("EXIST"),
            ErrorKind::InvalidInput | ErrorKind::NotFound => println!("PATH NOT FOUND"),
            _ => println!("CANNOT CREATE FILE"),
        }
        return;
    }
